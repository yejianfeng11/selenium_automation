...
public class StringUtils {
	...
	public static boolean isEquals(Map<String, String> srcMap,
		...
		for (String dkey : destSet) {
			if (srcMap.containsKey(dkey)) {
				for (String skey : srcSet) {
					if (skey.equalsIgnoreCase(dkey)) {
						...
						if (isEquals(expectedStr, str)) {
							System.out.println(skey + "期望值:" + expectedStr
									+ ";与实际值:" + str + "相等！");
							flag = true;
						} else {
							System.out.println(skey + "期望值:" + expectedStr
				...
				System.out.println("期望值中不存在" + dkey + "字段");
			}
	...
	public static boolean compareList(List<Map<String, String>> dbLines,
			List<Map<String, String>> showLines) {
		int i = 0;
		int j = 0;
		...
		for (i = 0; i < showLines.size(); i++) {
			for (j = 0; j < dbLines.size(); j++) {
				if (StringUtils.isEquals2(dbLines.get(j), showLines.get(i))) {
					System.out.println("期望值" + dbLines.get(j) + "；与实际值："
							+ showLines.get(i) + "相等！");
					result = true;
					dbLines.remove(j);
					break;
			...
			if (result == false) {
				System.out.println("期望值列表中不存在实际值：" + showLines.get(i));
				finalres = result;
			}
	...
	public static boolean isContain(String str1, String str2) {
		...
		if (res) {
			System.out.println("字符串校验成功：“" + str_longer + "”包含“" + str_shorter
					+ "”");
		} else {
			System.out.println("字符串校验失败：“" + str_longer + "”不包含“" + str_shorter
					+ "”");
		}
		return res;
	}
	 ...
	 * @return boolean
	 */
	public static boolean checkContentSame(List<String> list, List<String> list2) {
		boolean finalres = true;
		if (list.size() == list2.size()) {
			System.out.println("两个List长度相同");
		} else {
			System.out.println("两个List长度不相同");
			finalres = false;
		}
		for (int i = 0; i < list.size(); i++) {
			...
			for (int j = 0; j < list2.size(); j++) {
				if (list.get(i).equals(list2.get(j))) {
					isfind = true;
					System.out.println("找到" + list.get(i));
					list2.remove(j);
					break;
				}
			}
			...
			if (isfind) {
				...
				System.out.println("没有找到" + list.get(i));
			}
		}
		return finalres;
	}
